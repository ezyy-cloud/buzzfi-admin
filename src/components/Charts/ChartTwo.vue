...
const getWeekDates = (isCurrentWeek: boolean) => {
  const now = new Date();
...
const calculateAgentRevenue = (agent: any, date: Date) => {
  return salesStore.transactions
    .filter(
      (t: { user_id: any; created_at: string | number | Date }) =>
        t.user_id === agent.id &&
        new Date(t.created_at).toDateString() === date.toDateString()
    )
    .reduce(
      (sum: number, t) => sum + parseFloat(t.price) * JSON.parse(t.voucher_codes).length,
      0
    );
};

const chartData = computed(() => {
  const isCurrentWeek = selectedPeriod.value === "This Week";
  const weekDates = getWeekDates(isCurrentWeek);

  const series = agentStore.agents.map((agent: { name: any }) => ({
    name: agent.name,
    data: weekDates.map((date) => calculateAgentRevenue(agent, date)),
  }));
...
const apexOptions = computed(() => ({
  colors: agentStore.agents.map(
    (_: any, index: number) =>
      `hsl(${(index * 360) / agentStore.agents.length}, 70%, 50%)`
  ),
  chart: {
    type: "bar",
    height: 335,
...
<template>
  ...
  >
    <div class="mb-4 justify-between gap-4 sm:flex">
      <div>
        <h4 class="text-xl font-bold text-black dark:text-white">Vendor Sales</h4>
      </div>
      <div>
