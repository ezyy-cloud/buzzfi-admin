  ...
  <div
    ...
    <Modal :show="showEditModal" @close="showEditModal = false">
      ...
      <form @submit.prevent="updateAgentHandler">
        <div class="mb-4">
          ...
          <input v-model="editAgent.name"
            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
            id="edit-name" type="text" placeholder="Name" required>
        </div>
        <div class="mb-4">
          <label class="block text-gray-700 text-sm font-bold mb-2" for="edit-email">Email</label>
          <input v-model="editAgent.email"
            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
            id="edit-email" type="email" placeholder="Email" required>
        </div>
        <div class="flex justify-end">
    ...
    <Modal :show="showDeleteModal" @close="showDeleteModal = false">
      ...
      <div class="flex justify-end space-x-4">
        <button @click="deleteAgentConfirmed" class="bg-red-500 text-white py-2 px-4 rounded">Delete</button>
    ...
    <div v-if="!loading && !error" class="flex flex-col">
      ...
      <div v-for="(agent, key) in agents" :key="agent.id"
        :class="`grid grid-cols-3 sm:grid-cols-7 ${key === agents.length - 1 ? '' : 'border-b border-stroke dark:border-strokedark'}`">
...
const addAgentHandler = async () => {
  try {
    await agentsStore.addAgent({ name: name.value, email: email.value, password: password.value, password_confirmation: password.value });
